...
def _match_dynamic_quant(
    ...
    cached_bf16_bytes = ((n_cols + 127) // 128) * 128 * 2
    vlm_bytes = cached_bf16_bytes if cached_bf16_bytes <= 8192 else 0
    return GaudiProgram(
        name=name,
...
def _emit_dynamic_quant_tpc_c(program: GaudiProgram) -> TpcCSource:
    ...
    expected_vlm_bytes = ((n_cols + 127) // 128) * 128 * 2
    use_vlm_cache = expected_vlm_bytes <= 8192
    ...
    if use_vlm_cache:
        cache_declaration = f"""
#define TRITON_GAUDI_CACHE_CHUNKS {expected_vlm_bytes // 256}
__local__ bfloat128 triton_gaudi_values[TRITON_GAUDI_CACHE_CHUNKS];
"""
        cache_full_store = """
            triton_gaudi_values[lane / TRITON_GAUDI_BF16_LANES] = values;"""
        cache_tail_store = """
            triton_gaudi_values[full_columns / TRITON_GAUDI_BF16_LANES] = values;"""
        quantization_loads = """
